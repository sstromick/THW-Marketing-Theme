...
$employees = get_field('employees');
	...
	echo '				<ul id="employees" class="row">';

	$x = 1;

	foreach ($employees as $employee) {
		$employeesColClassesClear = null;

		if (in_array($x, $xsClearIndexes)) {
			$employeesColClassesClear .= ' col-xs-clear';
		}

		if (in_array($x, $smClearIndexes)) {
			$employeesColClassesClear .= ' col-sm-clear';
		}

		$x++;

		if (!empty($employee['name']) && !empty($employee['description'])) {
			echo '			<li class="' . $employeesColClasses . $employeesColClassesClear . '">
								<div class="employee">
